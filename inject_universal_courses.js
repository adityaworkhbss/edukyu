...
console.log('🚀 Universal Course Data Injection Script');
console.log('=======================================\n');
...
try {
    ...
    fs.writeFileSync('/workspaces/edukyu/src/Data Model/CoursePage/CoursePageData_final.jsx.backup.universal', courseDataContent);
    console.log('✅ Backup created\n');

    // Extract ALL course information with enhanced parsing
            ...
            if (university.university_info) {
                Object.keys(university.university_info).forEach(sectionKey => {
                    ...
                    if (section.course && section.course.description) {
                        ...
                        const parsedCourses = parseUniversalCourseDescription(section.course.description, universityKey);
    ...
    console.log(`💾 Backup location: CoursePageData_final.jsx.backup.universal`);
...
function parseUniversalCourseDescription(description, universityKey) {
    const courses = [];
...
function generateCourseStructure(course, courseKey) {
    ...
    // Use discounted fees if available, otherwise use original fees
    ...
    return `
        "${normalizedKey}": {
            "page": {
                ...
                "fees": {
                    "total": "₹${formatNumber(displayFees)}",
                    "perSemester": "₹${formatNumber(Math.round(displayFees / semesters))}"
                }
            },
            "specializations": [],
            "programBenefits": [
...
console.log('\n🎯 Universal course injection finished!');