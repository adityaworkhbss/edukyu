...
console.log('🚀 Enhanced Course Data Injection Script');
console.log('=========================================\n');
...
try {
    ...
    const universityMatches = extractedCourses.map(courseInfo => {
        // Check if university exists in course data
        const universityPattern = new RegExp(`"${courseInfo.universityKey}":\\s*{`, 'g');
    ...
    console.log('\n🛠️  Grouping courses by university...');
    ...
    Object.keys(coursesByUniversity).forEach(universityKey => {
        ...
        courses.forEach(course => {
            ...
            // Find the university section and inject the course
            const universityRegex = new RegExp(
                `("${universityKey}":\\s*{)([\\s\\S]*?)(\\s*})(\\s*(?:,|\\]))`,
                'g'
            );

            const match = universityRegex.exec(modifiedContent);
            if (match) {
                // Check if this course already exists
                const existingCoursePattern = new RegExp(`"${courseKey}":`);
                if (!existingCoursePattern.test(match[2])) {

                    const replacement = `${match[1]}${match[2]},${courseStructure}${match[3]}${match[4]}`;
                    modifiedContent = modifiedContent.replace(universityRegex, replacement);
                    totalInjections++;
    ...
    try {
        ...
        // Check structure
        const dataMatch = modifiedContent.match(/export const CoursePageData = (\[[\s\S]*\]);?$/);
        if (dataMatch) {
            console.log('✅ CoursePageData structure maintained');
        } else {
            console.log('❌ CoursePageData structure may be corrupted');
        }
...
console.log('\n🎯 Enhanced course extraction and injection completed!');