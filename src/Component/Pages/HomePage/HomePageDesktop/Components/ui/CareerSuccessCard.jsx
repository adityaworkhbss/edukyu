...
const CareerSuccessCard = ({ data, isActive = false }) => {
    const containerClass = isActive
        ? 'bg-[#024B53] text-white'
        : 'bg-white text-[#024B53] border border-[#024B53]';

    const iconFill = isActive ? '#FFFFFF' : '#024B53';

    return (
        <div className={`${containerClass} rounded-[22px] p-6 flex gap-8 items-start flex-col `}>
            {/* Left Section */}
            <div className="flex flex-row items-start gap-[50px] justify-between">
                <div className="flex flex-row items-center gap-4">
                    ...
                    />
                    <div>
                        <div className={`text-[20px] font-[600] font-[Outfit] leading-[normal] ${isActive ? '' : 'text-[#024B53]'}`} style={isActive ? { color: 'var(--Colour-Secondary-Colours-Mustard---400, #FFD23F)' } : {}}>
                            {data.name}
                        </div>

                        <div className={`${isActive ? 'text-white' : 'text-[#024B53]'} text-[14px] font-[400] leading-[20px] font-[Outfit] lowercase`}>
            ...
            <div className="relative w-full flex flex-col">
                {/* Quotation mark */}
                <div
                    className="absolute -top-[20px] left-[24px] -translate-x-1/2 text-[110px] font-semibold font-[Outfit] leading-none"
                    style={{ color: isActive ? 'rgba(255,255,255,0.30)' : 'rgba(2,75,83,0.08)' }}
                >
                    “
                </div>

                {/* Testimonial text */}
                <div className={`${isActive ? 'text-white' : 'text-[#024B53]'} text-[14px] pt-[40px] font-normal font-[Outfit] text-center leading-normal px-4`}>
                    {data.content}
                </div>
            </div>
